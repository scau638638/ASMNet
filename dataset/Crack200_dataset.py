...
class Crack200Dataset(Dataset):
    def __init__(self, dataset_root, txt_name: str = 'train', augment=None):
        super(Crack200Dataset, self).__init__()
        ...
        pic_dir = os.path.join(dataset_root, 'data')

        with open(txt_path, 'r') as fp:
            name_list = [name.strip() for name in fp if len(name.strip()) > 0]

        self.images = [os.path.join(pic_dir, name + '.jpg') for name in name_list]
        self.gt = [os.path.join(pic_dir, name + '.png') for name in name_list]
