...
class CRKWH100Dataset(Dataset):
    def __init__(self, dataset_root, txt_name: str = 'train', augment=None):
        super(CRKWH100Dataset, self).__init__()
        ...
        gt_dir = os.path.join(dataset_root, 'groundTruth')

        with open(txt_path, 'r') as fp:
            name_list = [name.strip() for name in fp if len(name.strip()) > 0]

        self.images = [os.path.join(images_dir, name + '.png') for name in name_list]
