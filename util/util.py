...
def save_population_ckpt(population, fitness, exp_name, g):
    import pickle
    ...
    # pickle.dump(fitness, pickle_file1)
    pickle_file1.close()
...
def get_gene_len(de_func_type, en_func_type, de_node_num_list, en_node_num_list, layer_num_list, de_node_num, en_node_num, only_en=False):
    ...
    de_att_location_gene_len = int(np.ceil(np.log2(de_node_num)))
    en_att_location_gene_len = int(np.ceil(np.log2(en_node_num)))

    de_op_gene_len = de_node_func_gene_len + de_att_location_gene_len + 1
    en_op_gene_len = en_node_func_gene_len + en_att_location_gene_len + 1
    ...
    for i in range(len(de_node_num_list)):
        de_gene_len_list[i] = de_op_gene_len + de_connect_gene_len_list[i]
    for i in range(len(en_node_num_list)):
        en_gene_len_list[i] = en_op_gene_len + en_connect_gene_len_list[i]

    if only_en:
        gene_len = sum(en_gene_len_list) + layer_num_gene_len
    else:
        gene_len = sum(de_gene_len_list) + sum(en_gene_len_list) + layer_num_gene_len
